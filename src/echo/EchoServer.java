//에코서버란, 클라이언트의 메세지를 받은 후,그대로 전달하는 역할의
//네트워크 서버를 의미!!(채팅 제작의 기초)
//소켓이란? 전산에서의 소켓도 현실에서의 소켓처럼 이미 내부적으로
//	기술적인 부분을 구현해 놓은 구현체를 말한다..
...
public class EchoServer {
	ServerSocket server;
	int port=7777;//0~1023까지는 시스텀 점유 포트임으로 사용하지말자
	...
	public EchoServer() {
		try {
			...
			//Socket은 대화를 나눌수 있는 전화기와 같다!!
			
			Socket client=server.accept();//접속자 기다림..메인쓰레드인 실행부는
			//접속자가 발생할때까지 대기상태에빠짐..지연..
			System.out.println("접속자발견!");
			
			//클라이언트가 보낸 메세지 받을려면..실행중인 프로그램 
